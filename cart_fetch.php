<?php
	...
	if(isset($_SESSION['user'])){
		try{
			$stmt = $conn->prepare("SELECT products.id, products.photo, products.name AS prodname, category.name AS catname ,cart.user_id FROM cart LEFT JOIN products ON products.id=cart.product_id LEFT JOIN category ON category.id=products.category_id WHERE user_id=:user_id");
